...
- **Every "+ Add X" toolbar button uses the shared `components/AddButton`** (Shleeji mirrors it with its own `components/AddButton.js`), rendered inside a right-aligned `__toolbar` row at the top of the view. Pass `ariaLabel` exactly as e2e targets it (`"Add header"`, `"Add goal"`, …) — the accessible name is a test contract. New views/panels must follow this pattern.
...
- **Goals mirror the todo's presentation on purpose**: `GoalsPanel` renders step rows with the todo's own `.task-card*` markup and imports `TaskCard/TaskCard.css` directly, and `AddStepModal` reuses `AddTaskModal.css` and its `add-modal__*` classes. Changing those todo styles changes the Goals view too — that shared-source coupling is the point, so restyle both together rather than forking a copy. Panel-specific hooks (`goals-panel__step-row`, `goals-panel__step-name`, `goals-panel__step-status`) stay on the rows for e2e.
