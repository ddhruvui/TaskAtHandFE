...
- **View modes** (App.tsx state): default, By Date (groups by `getEcdDateKey`; done tasks excluded; recurring surfaces under today when due; ordered today → past → future with a thick `.bydate-divider` between the present/past/future sections; "No date" group last), Insights, Events, Goals, Projects, Affirmations.
